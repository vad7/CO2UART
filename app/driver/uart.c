...
#ifdef USE_UART1
void ICACHE_FLASH_ATTR
uart_init(UartBautRate uart0_br, UartBautRate uart1_br, uart_recieve_func * rx_task)
#else
void ICACHE_FLASH_ATTR
uart_init(UartBautRate uart0_br, uart_recieve_func * rx_task)
#endif
{
    /*this is a example to process uart data from task,please change the priority to fit your application task if exists*/
    system_os_task(rx_task, uart_recvTaskPrio, uart_recvTaskQueue, uart_recvTaskQueueLen);  //demo with a task to process the uart data
